...
class Trainer(object):
    def __init__(self, config, model, optimizer, dataloader, dataloader_val,
        ...
        self.save_every = 10  # 每10个epoch save一下
        self.log_step = 100
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, data in enumerate(self.data_loader):
            ...
            # input_to_model = {}
            # input_to_model['pose'] = stickman.to(self.device)
            # input_to_model['appearance'] = im.to(self.device)
            # input_to_model['target'] = im.to(self.device)
            ...
            # 损失函数总和
            instance_total_loss = 0
            for key in instance_losses.keys():
                instance_total_loss += self.config["losses"][key]["weight"] * instance_losses[key]
            instance_losses["total"] = instance_total_loss
            ...
            if batch_idx % self.log_step == 99:
                self.log_timer.update(time.time() - _tic)
                _tic = time.time()
                print("Train Epoch: %d, loss:%.4f, time used: %.4f (per %d batches)" % (
                    epoch,
                    losses['batch']['total'].item(),
                    self.log_timer.avg,
                    self.log_step))
                self.writer.add_scalar("train_loss/kl", batch_losses["KL"],
                                       global_step=batch_idx + self.len_epoch * epoch)
                self.writer.add_scalar("train_loss/vgg", batch_losses["vgg"],
                                       global_step=batch_idx + self.len_epoch * epoch)
                self.writer.add_scalar("train_loss/l1", batch_losses["l1"],
                                       global_step=batch_idx + self.len_epoch * epoch)
                self.writer.add_image("train_image/pose",
                                      adjust_support(stickman[0].cpu().numpy(), "0->1", "-1->1")[::-1, :, :],
                                      global_step=batch_idx + self.len_epoch * epoch)
                self.writer.add_image("train_image/target",
                                      adjust_support(im[0].cpu().numpy(), "0->1", "-1->1")[::-1, :, :],
                                      global_step=batch_idx + self.len_epoch * epoch)
                self.writer.add_image("train_image/pred",
                                      adjust_support(model_output["pred"][0].cpu().detach().numpy(), "0->1", "-1->1")[
                                      ::-1, :, :],
                                      global_step=batch_idx + self.len_epoch * epoch)
        if self.lr_scheduler is not None:
            self.lr_scheduler.step()
        print("============Epoch %d done! Time used: %.4f================" % (
            epoch+1, self.log_timer.avg * self.len_epoch / self.log_step))

        if epoch % self.save_every == 9:
            self.save(self.ckpt_path + "%d" % (epoch+1))
            print("============Ckpt saved at %s!================" % (self.ckpt_path + "%d" %  (epoch+1)))
    ...
    def _val_epoch(self, epoch):
        ...
        for batch_idx, data in enumerate(self.data_loader_val):
            ...
            if "l1" in self.config["losses"].keys():
                ...
                l1_loss.update(instance_losses["l1"].mean())
            # KL 散度
            ...
            if "KL" in self.config["losses"].keys() and "q_means" in model_output.keys():
                ...
                kl_loss.update(instance_losses["KL"].mean())
            # vgg loss
            if "vgg" in self.config["losses"]:
                instance_losses["vgg"] = self.vgg_losser(
                    model_output["pred"], input_to_model["x"]
                )
                vgg_loss.update(instance_losses["vgg"].mean())
            # 损失函数总和
            instance_total_loss = 0
            for key in instance_losses.keys():
                instance_total_loss += self.config["losses"][key]["weight"] * instance_losses[key]
            ...
            total_loss.update(instance_losses["total"].mean())
        ...
        self.writer.add_image("eval_image/pose", adjust_support(rnd_output_verbose["pose"][0].cpu().numpy(),"0->1","-1->1")[::-1,:,:],
                              global_step=epoch)
        self.writer.add_image("eval_image/target", adjust_support(rnd_output_verbose["target"][0].cpu().numpy(),"0->1","-1->1")[::-1,:,:],
                              global_step=epoch)
        self.writer.add_image("eval_image/pred", adjust_support(rnd_output_verbose["pred"][0].cpu().detach().numpy(),"0->1","-1->1")[::-1,:,:],
                              global_step=epoch)
